...
public class UserServiceImpl implements IUserService {
    ...
    //这个checkValid方法是在注册用户的时候写的，即检查当前用户名或者邮箱是否已经被注册
    //type有可能是email或者username,从而决定此方法是校验email还是username
    ...
    //这个方法之前会先验证session,验证通过才会调用这个方法
    //防止横向越权，要校验一下这个用户的旧密码，一定要指定这个用户
    //session的验证并不需要手动传参，如果不指定，用户可以更改
    //其他任何人的账户和密码
    ...
    //更新个人信息
    //注意此处的校验逻辑，对于该user的新信息，只判断了email，对于
    //其他信息并未校验
    public ServerResponse<User> updateInformation(User user){
        ...
        if(resultCount>0){
            return ServerResponse.createByErrorMessage("email已被占用，请更换email");
